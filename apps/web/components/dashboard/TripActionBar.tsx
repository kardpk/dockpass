...
import {
  ...
  Share2, Link as LinkIcon, RefreshCw, X,
} from 'lucide-react'
...
export function TripActionBar({ tripId, tripSlug, status, allWaiversSigned }: TripActionBarProps) {
  ...
  async function generateSnapshot() {
    ...
    try {
      ...
      setSnapshotError(err instanceof Error ? err.message : 'Failed to generate captain link.')
    ...
    <div>
          ...
          <div className="td-panel">
            <div className="td-panel-header">
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <LinkIcon size={11} strokeWidth={2} style={{ color: 'var(--td-gold-dim)' }} />
            ...
            <div className="td-btn-split td-panel-footer">
              ...
              <a
                href={tripLink} target="_blank" rel="noopener noreferrer"
          ...
          {snapshotUrl ? (
            <div className="td-panel">
              <div className="td-panel-header">
                ...
                <span style={{ fontFamily: 'var(--td-mono)', fontSize: 10, color: 'var(--td-warn)', display: 'flex', alignItems: 'center', gap: 4 }}>
                  Active
                </span>
              </div>
              ...
              <div className="td-btn-split td-panel-footer">
                <button onClick={copySnapshot} className={`td-btn-split-item ${copiedSnapshot ? 'copied' : ''}`}>
                  {copiedSnapshot ? <Check size={12} strokeWidth={2.5} /> : <Copy size={12} strokeWidth={2} />}
                  {copiedSnapshot ? 'Copied' : 'Copy link'}
                </button>
                <button
                  onClick={generateSnapshot} disabled={generatingSnapshot}
                  className="td-btn-split-item"
                  style={{ color: 'var(--td-err)' }}
                >
          ...
          {snapshotError && (
            <div className="td-alert td-alert-err">
              ...
              <button
                ...
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--td-err)', flexShrink: 0, padding: 0 }}
              >
      ...
      {showDocs && (
        <section>
          ...
          {downloadError && (
            <div className="td-alert td-alert-err" style={{ marginBottom: 10 }}>
          ...
          <button
            ...
            className="td-doc-row"
            style={{ opacity: downloadingPdf ? 0.5 : 1 }}
          >
            <Download size={18} strokeWidth={1.8} className="td-doc-icon" />
            <div style={{ flex: 1, minWidth: 0, textAlign: 'left' }}>
              <p className="td-doc-title">
          ...
          {showUscg ? (
            <button
              onClick={downloadUscgCsv} disabled={downloadingCsv}
              className="td-doc-row td-doc-row-gold"
              style={{ opacity: downloadingCsv ? 0.5 : 1 }}
            >
              <Anchor size={18} strokeWidth={1.8} className="td-doc-icon td-doc-icon-gold" />
              <div style={{ flex: 1, minWidth: 0, textAlign: 'left' }}>
                <p className="td-doc-title">
            ...
            <div className="td-doc-row locked">
              <Anchor size={18} strokeWidth={1.8} className="td-doc-icon" />
